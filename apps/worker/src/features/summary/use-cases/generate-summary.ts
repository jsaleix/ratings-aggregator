...
{
  "content": "Your concise summary of the reception (MAX. 200 chars).",
  "score": "A letter grade (from A+ to F) based on the ratings."
}
...
Do not include anything else in your reply.`;
    ...
    async execute(movieId: string) {
        ...
        const { content } = await this.aiService.sendRequest({
            user: userPrompt,
            system: systemPrompt,
        });

        return await this.dbService.saveSummary({ movieId, content });
    }
}
